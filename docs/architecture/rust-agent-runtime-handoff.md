...
- The embedded FRB path currently covers deterministic native step contracts
  ...
  agent path, the HealthOS Recovery Alert / Weekly Summary tool-using paths,
...
removed; the remaining direct provider surface is limited to low-level
runtime/provider implementation and focused tests.
