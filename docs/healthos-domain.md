...
- ✅ **D-2.7 D-2 收尾**: 真 UI + sleep 合并 + briefing 链路 (2026-05-27) — (1) **iOS sleep segment 合并**:在 `health_platform_adapter.dart` 加纯函数 `mergeSleepSegments(segments, gap: 30min)`,把 HealthKit 多 segment (3–7 行/晚) 合成单 session;merged id 形如 `hk:sleep:merged:<isoStart>` 保 idempotent,Android Health Connect (整 session) 实际不会触发合并(gap > 30 min)。单 segment 通过时保留原 `externalId` 不动。`_io.dart` 在 `fetchRange` 收集 sleep 后过一次 merge。(2) **briefing 跑前先 sync**:`runMorningBriefingNow` 在跑 agent 前 `await healthSyncServiceProvider.future` + `syncRange()`(swallow 异常,stale 总比无 briefing 好),解决"workmanager 起床但读不到新数据"的洞;背景 isolate callback 仍保持轻量 (`kMorningBriefingDueAtKey` flag + 占位通知),只有前台路径加 sync 重活。(3) **Today UI**:`HealthTodayPage` 加 4 卡片网格 (`Sleep / HRV / Recovery / Workout`,2x2,FCard + 大值 + relative ago) 在 briefing 卡片上方,数据走 `latestSleepSessionProvider` / `latestHrvProvider` / `latestWorkoutProvider` / `recoverySignalProvider`(全部 autoDispose,opt-in OFF 返 null);recovery 卡 verdict 色编码 (`rested`→primary / `balanced`→outline / `strained`→error)。(4) **Trend UI**:新 `HealthTrendPage` 三个 `_TrendCard` (HRV / Sleep hours / Workout minutes/day),走 `NwLineChart` + `trendChartProvider.family<List<ChartPoint>, HealthMetricKind>`;纯映射 `healthTrendProject(rows, kind, cutoff)` 拆出来给 unit test,workout 按 UTC 日聚合 sum minutes,sleep 单位 s→h,< 2 点显示 "Not enough data yet"。(5) **Plan UI**:新 `HealthPlanPage` 一张 recovery 卡(verdict icon + score + 中文一句建议 + 四个 input rows: HRV/Sleep/RHR/VO₂max)+ 一张 disclaimer "不是医学诊断"。Verdict→suggestion 静态映射 (`rested→可推高强度` / `balanced→维持节奏` / `strained→减负`)。(6) `health_routes.dart` 把 Trend / Plan branches 切到新页;`health_placeholder_page.dart` 删除(零引用)。新测试:`merge_sleep_segments_test.dart` 7 个(empty / single / merge / nap-not-merged / idempotent / platform-prefix / out-of-order)+ `health_trend_project_test.dart` 5 个(hrv ascending / sleep-hours / workout-per-day-sum / cutoff / unknown→empty);全项目 79 个 health 测试 + `flutter analyze --fatal-infos` clean。**仍未做**:per-kind opt-in,设置页同步状态显示,加密备份覆盖 health 行验证 — 留给 §11 follow-up。
...
## 5. IA placement (D-2.3 seam + D-2.7 实页)

HealthOS 内部 tabs(在 Option B domain dock 下):

- **Today** — 4 卡片网格(Sleep / HRV / Recovery / Workout)+ Morning Briefing 卡 + Run-now 按钮(D-2.7)
- **Trend** — HRV / Sleep hours / Workout minutes 三条最近 30 天 line chart(D-2.7)
- **Plan** — Recovery 卡(verdict + score + 中文建议 + HRV/Sleep/RHR/VO₂max 输入行)+ 医学免责声明(D-2.7;只显示,不自动调度,§10 反目标)
