...
- Agent artifact notification / route deep link 已通过 `readActiveAgentArtifact` 统一校验 current user、active domain、active agent registry、expected domain 和 visibility state，防止 query 参数绕过领域开关、用户边界或 dismissed / snoozed / expired 状态直接打开历史 artifact。
...
- Production DomainPack composition 已覆盖所有 active production agent 必须在 agent outcome corpus 中有同域 ready outcome，防止新增 agent 进入用户界面但没有真实可见结果回归。
- Agent runtime catalog 已覆盖 active-domain agent filtering，并以生产 DomainPack 校验 runtime 暴露的 agent / tool / proposal kind / prompt block 与 active composition 一致，防止 native/runtime 看到 stale agent 或漂移的注册表。
...
- Regression corpus 已覆盖 FinanceOS 首批 agent，校验所有 corpus agent 都有 fixture 文件，且每个 corpus case id 都必须出现在可执行 evaluator fixture 中；Finance / Health / Execution / Knowledge 的真实 agent fixture 均已通过 domain-neutral evaluator 接入，`knowledge_routine_due.domain_opt_out` 已接到生产 domain opt-in composition 层面的可执行 no-run eval，`execution.review.budget_exhausted` 已接到真实 runner failed-outcome eval。
- Regression evaluator 已覆盖 ready artifact 的统一 action kind / intent，并拒绝 unexpected artifact action，防止各 domain agent 产出无动作、私有动作模型或绕过统一 follow-up/proposal surface。
- Regression corpus contract 已要求所有 ready cases 同时声明 action kind 和 action intent，防止新增 agent 只记录本地动作而没有统一 follow-up 入口。
- Regression corpus contract 已要求所有 ready action intents 必须注册到 agent intent descriptors、允许对应 domain、并支持 `agent_artifact` 对象。
- Regression evaluator 已覆盖带 intent 的 artifact actions 必须显式指向当前 `agent_artifact`，防止 follow-up action 丢失对象或绑定到错误对象。
- Regression evaluator 已覆盖 ready result / artifact 的 trace id 必须成对一致，防止可见 agent 结果指向错误或断开的本地透明度链路。
