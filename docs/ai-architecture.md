...
> **Wave 进度**: Wave 1–40 完成（Wave 40 = `AiTouchMark` 公共化 + 3 个 detail page 接入：AccountForm / CashForm / ActivityEntryDetail）— 详见 §8 实现状态表与 Wave 落地清单。**§5 Interaction Grammar 整章及配套基础设施全部完成；AiTouchMark 在所有四个 entity type 上的关键 detail page 已覆盖**。剩余主线: 其他 Asset 子类型表单（deposit / wealth_product / equity_asset_detail）类似接入（pattern 完全相同，5 行改动 / page）；Phase 5 端侧 LLM、`tools.rs` 进一步细分（Wave 25 仅完成 xirr 提取 + 目录化）、long-window `subscription_changes`、Analytical 层 P2 四模型；可选 swipe-to-apply 真实手势 wave。
