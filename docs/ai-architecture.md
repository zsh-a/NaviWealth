...
**W-D7 后（cloud relay 已删除，现状）**：无 cloud 回落。web 平台 / 无可用 active profile（Wave 46：profiles 为空或 active 无 key）/ provider 报错（鉴权失败、网络）→ `RuntimeRoutingAiChatApiClient` 直接产 `device_unavailable` `ErrorEvent`+`DoneEvent`，UI 引导用户在设置中添加自带 API Key（web 不支持设备侧 AI）。无静默 device→cloud 失效转移（已不存在）；device 自身的 provider 报错按其 `ErrorEvent`/`DoneEvent` 原样透传，不再被 cloud 吞掉。降级仍写 AiTrace（`terminalReason` 体现）。

> 历史（W-D1~W-D6 共存窗口，已失效）：`DeviceLlmRuntime` 曾在上述情况 buffer-until-content 回落 `CloudAnthropicRuntime`。该路径随 W-D7 删除。
...
- **端侧 LLM 必须 opt-in** — 200MB 级别下载不应阻塞首屏。~~用户自带 key 路径同样 opt-in（未填 key 不改变现状）~~ **Wave 46 修正**：自带 key 路径的显式 `enabled` 开关已删除——W-D7 删 cloud 后无静默回落可破坏，故"有 active profile = 用，无 = `device_unavailable`"本身即是 opt-in（未添加 key 不改变现状），多一个开关纯属冗余。
...
- **为什么先并存再删 cloud AI**（§4.6 / §8 / §9 W-D7）— 原计划：140 backend tests + 已验证的 read model 主通道是资产；`AiRuntime` registry 让端云并存零成本，端侧路径生产验证稳定后再单独 Wave 做不可逆删除，降低回归风险。**实际：W-D7 经用户明确决策提前执行（早于 W-D4.2c 4/4 完成、未待生产验证）**，知情接受其代价（`compute_xirr`/`get_xirr_summary`/`compute_net_worth` 失去唯一实现；web 失去 AI；cloud 失效转移移除）。后续若补端侧化这三个工具，须自带数值正确性测试——已无 backend 可做 roundtrip parity（见下条）。
