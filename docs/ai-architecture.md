...
**W-D7 后（cloud relay 已删除，现状）**：无 cloud 回落。web 平台 / 无 key / 未 opt-in / provider 报错（鉴权失败、网络）→ `RuntimeRoutingAiChatApiClient` 直接产 `device_unavailable` `ErrorEvent`+`DoneEvent`，UI 引导用户在设置中配置自带 API Key（web 不支持设备侧 AI）。无静默 device→cloud 失效转移（已不存在）；device 自身的 provider 报错按其 `ErrorEvent`/`DoneEvent` 原样透传，不再被 cloud 吞掉。降级仍写 AiTrace（`terminalReason` 体现）。
