//! Analytical 层 P1 — `transfer_links` (device-sourced)。
//!
//! 端侧 `transferMatcher` (Wave 2-B) 把 "账户 A 出 / 账户 B 进" 配成
//! 一对，通过 ContextPack.analytical_uploads (kind='transfer_link')
//! 上报。本模块镜像入表。
//!
//! payload schema (v1):
//!   { from_txn_id, to_txn_id, amount_minor, currency }
//!
//! 与 refund_links 同构 (Wave 16 同源)。
...
pub const NAME: &str = "transfer_links";
pub const SCHEMA_VERSION: u32 = 1;
pub const CALCULATION_VERSION: u32 = 1;

#[derive(Debug, Clone)]
pub struct TransferLinkUpload<'a> {
    pub id: &'a str,
    pub payload: &'a Value,
...
#[derive(Debug, Clone)]
pub struct TransferLinkRow {
    pub id: String,
    pub from_txn_id: Option<String>,
    pub to_txn_id: Option<String>,
...
pub async fn ingest(
    ...
    let stmts: Result<Vec<_>, AppError> = uploads
        ...
        .map(|u| {
            ...
            let from_id = payload_str(u.payload, "from_txn_id");
            let to_id = payload_str(u.payload, "to_txn_id");
            let amount = payload_amount_minor(u.payload, "amount_minor");
            let currency = payload_str(u.payload, "currency");
            db.prepare(
                "INSERT INTO read_model_transfer_links
                    (user_id, id, payload, from_txn_id, to_txn_id,
...
pub async fn query_all(
    ...
    user_id: &str,
    currency: Option<&str>,
) -> Result<Vec<TransferLinkRow>, AppError> {
    let stmt = match currency {
        Some(c) => db
            .prepare(
                "SELECT id, payload, from_txn_id, to_txn_id, amount_minor, currency
                 FROM read_model_transfer_links
...
struct RowSelect {
    id: String,
    payload: String,
    from_txn_id: Option<String>,
    to_txn_id: Option<String>,
