...
#
# Matching .ttf subsets are emitted for Flutter's native golden-test renderer,
# which cannot decode WOFF2. They are not declared in pubspec.yaml and are not
# included in production bundles.
#
# All files are build artifacts: regenerated by this script and gitignored,
...
echo "subsetting native golden font → $BASE_TEST_OUT"
"$PYFTSUBSET" "$SOURCE_FONT" \
  --unicodes-file="$BASE_UNICODES" \
  --output-file="$BASE_TEST_OUT" \
  "${common_args[@]}"

echo "subsetting native golden font → $EXT_TEST_OUT"
"$PYFTSUBSET" "$SOURCE_FONT" \
  --unicodes-file="$EXT_UNICODES" \
  --output-file="$EXT_TEST_OUT" \
  "${common_args[@]}"
