...
> **实施进度(2026-07-26)**:P0 ✅(4 个 lint 护栏进 CI,`DESIGN.md` 已删)· P1 ✅(`AppThemeData`/`ColorRole`/`resolveAppTheme`/`context.appTheme` 落地,108 项对比度不变量测试,4 项既有色板债入豁免表)· P2 ✅(§3.6 四个 bug 修复;`SignedMoneyText`/Health 趋势接 `MarketColors`;badge/banner 换 `on*Container`;手拼百分比 19→0;SnackBar/Material Divider/页面级 spinner 清零;`KnowledgeEmptyState` 补 `action`;zh ARB 知识类型补译)。P3 部分 ✅(静默加载 10 处判定完毕——4 处换骨架、7 处标注为有意为空;裸错误态 7 处接 `AppEmptyState.error`/`userSafeErrorMessage`;Material spinner 清零并锁 0;sheet 桌面限宽 720)· P4 核心 ✅(`shellDesktop` 并入 1240;切换 chip 在 <1240 常驻、平板侧栏新增切域与 Ask-AI 入口;撤销横幅上移至全部布局;`Cmd/Ctrl-1..N` 域内化)· P5 部分 ✅(浅色 info 前景 cyanBrand700、色盲浅色 down 前景 cbOrangeDark,对比度豁免表清零)。P5 主体 ✅(73 处旧主题入口迁至 `context.appTheme`,`ColorRole` 补 `onFg`;剩余 8 处 = 6 处 `MarketColors.of` 偏好关键部件——与 `MarketColorsScope` 一并退役——加 2 处 `AccentColors.series` 图表常量,lint 基线锁 8)· P4 返回协议 ✅(非首 tab 根先回本域首 tab 再退出确认)· `/life` 补下拉刷新与 1200 限宽。MarketColorsScope 退役 ✅(最后 6 处 `MarketColors.of` 迁至 `context.appTheme.market`,scope/provider/`MarketColors.of` 删除,基线锁 2 = 仅 `AccentColors.series`)· tokens.json 导出管线 ✅(Dart → JSON 单向,406 token,`check-design-tokens-export.sh` 进 CI)· 未开通域深链提示 ✅(`?blocked=` + 说明横幅)· 死部件删除 ✅(`AppSidePanel`/`AppClosePageScaffold`/`DomainAiPromptBar` 部件,保留在用的 `DomainAiPromptAction` 值类)。旧表折叠 ✅(`SemanticColors.of` 上下文查找删除,与 `MarketColors` 同构——常量表仅作为 `resolveAppTheme` 输入存活)· raw `.when` 收编 ✅(标准形态迁 `whenOrLoading`/`kDefaultError`,详见提交记录)。**Scaffold 8→3 重新判定**:审查所列 8 种中,`AppTaskScaffold`/`ObjectDetailScaffold`/`AppFormPageScaffold` 实为基于 `AppPageScaffold` 的分层模板(组合而非竞争),予以保留;真正的竞争实现(`AppClosePageScaffold`、死件 `AppSidePanel`)已删除;`DomainTabScaffold` vs `ShellTabScaffold` 的归并留待专项。**预设主题第一批 ✅**:新增 `AppSurfaceStyle {standard, oled, highContrast}` 第三偏好轴(与明暗、市场色正交)——OLED 纯黑面(深色专属,浅色回退标准)与高对比风格(内容梯度 AAA 7:1、状态/市场前景 4.5:1,经色相保持的 HSL 增强);`ThemeInputs` 扩维后对比度不变量测试自动扩至 324 项;Forui 表面同步、SharedPreferences 持久化、设置页外观区新增选择行、tokens.json 新增 `color.oled` 组。**第二批 AccentSeed ✅**:第四正交轴 `AppAccentSeed {cyan, violet, indigo}`——`AccentSeedSlots` 语义槽位表(cyan 与旧品牌色像素一致),贯通 resolver/Forui/Material 三处 primary;与市场/状态语义冲突的色相(绿/红/琥珀/橙)有意不提供;accent 角色新增逐 seed × 亮度 × 风格的对比度不变量(共 354 项);violet/indigo 色阶入 L0 并导出 tokens.json。命名组合包评估后**不做**:两轴设置行已各自可发现,捆绑预设收益有限,留作未来 polish 选项。剩余:排版比例尺统一(§4,两套 scale 合一——影响全局渲染,需带 golden 基线更新的专项)、`/life` 进壳已由"切换 chip 全宽可见 + 桌面 dock"覆盖(底栏属设计取舍,不强加)、`AccentColors.series` 2 处图表常量(可接受的稳定余量)。注:P1 聚合对象命名为 `AppThemeData`(避让既有 Material 工厂类 `AppTheme`)。

> **增量进度(2026-07-26 · 质感专项)**:排版比例尺合一 ✅(`AppTypeScale` 落地 `theme.type`,Forui `FTypeface` 槽位由 `kAppTypefaceSlots` 单表构建——移动端字号收敛到文档比例尺,touch/desktop 隐式 ±2px 分叉删除并记为显式决策;`type.chartCaption` 收编图表 6 处裸 `fontSize:10`)· L2 组件规格层 ✅(`theme.press/card/badge/divider/metricTile`;按压系数 0.985/0.97/0.96/0.99 → 单值 0.98;`SoftCard.borderless` 参数删除,§8.3 描边策略 level 内建:raised=无边框+阴影;s14 卡片内边距清零入 `cardPadding/densePadding`;`_OpportunityCard` 内层 Padding 反模式修复;grouped-surface 填充单源 `appGroupedSurfaceFill`)· 分类色 ✅(`theme.categorical` 按明暗自适应 seed 色,expense/knowledge/health 三族渲染点全部过 `adapt()`;`health.body` violet→pink 解决语义冲撞;`ExpenseCategoryColors.cart` 删除)· 色阶 ✅(`navy300` 重推导回青灰 navy 族 `#90A6AD`)· 全局转场 ✅(`AppPageTransitionsBuilder` 进 `pageTransitionsTheme`,~80 条 `GoRoute(builder:)` 全部获得宽度感知转场;`buildHeroAwareTransitionPage` 及 3 个逐路由接线删除;iOS 保留 Cupertino 滑返)· 动效落地 ✅(`AnimatedMoneyText` 方向色 12% 变化脉冲;`NwLineChart` 首帧 600ms 左→右揭示;activity/持仓首帧 stagger;`AppRefreshIndicator` 统一下拉刷新,`KnowledgePullToRefresh` 删除)· 交互语法 ✅(39 处直接 `Haptics.*` 收口 `AppInteraction.signal`;`AppMotionPolicy` 按角色分级——transition 减半保留 cross-fade、decorative/status 归零;`SoftCard` 键盘焦点环 + Enter/Space 激活 + click 光标)。文档同步:doc 11 重写对齐实现;`design_tokens/README.md` 迁 `context.appTheme`。

> **增量进度(2026-07-27 · 质感专项二)**:P1' 预设接线补全 ✅(`SoftCard`/吸顶条/玻璃 dock/氛围/刷新指示器改消费 `theme.surfaces`——OLED 触达 raised/hero;Material `ThemeData` 接 `surfaceStyle`,`scaffoldBackground`/`ColorScheme` 随 OLED/高对比;玻璃色公式单源 `appGlassDecoration`,桌面 sidebar 获得同一导航材质)· P2' 反馈体系 ✅(壳层 `SyncActivityStrip` 细线进度+完成/失败闪烁;ingest 批量提交 ≥2 笔改总结面 `showIngestSummarySheet`(✓ 缩放入场 + 计数滚动);FIRE 25/50/75/100% 里程碑一次性庆祝 `fire_milestone_celebration.dart`;空状态死路 26→0(4 处入 documented allowlist),护栏 `empty_state_cta_test.dart` 锁死)· P3' 桌面公民权 ✅(`AppTappable` 默认焦点环 + Enter/Space,89 处裸 `FTappable` 迁移;`AppIconButton` hover 态 + 焦点环)· P4' §8.5 Agent 面板统一 ✅(`AgentResultsPanel` 法定四态,四域三份 `_*AgentPanelFrame` 删除,meta 一律 relativeTime;Health 空态接 `AgentEmptyStateCard`)· 滑动语法 ✅(`AppDismissible` 统一背景/阈值/触觉/确认语义,4 处方言收编,activity feed 行新增滑动删除)· P5' §8.1/§8.2 ✅(wealth_hub 迁 `BriefScaffold`;knowledge 三 tab 补氛围/统一节奏;FAB 全 App 退役——`AppFloatingActionSurface`/`KnowledgeFloatingActionMotion`/`shellTabFloatingActionBottom` 删除,library/review 改页头 action;Execution Today 补 display 档 stage 数字,Health 恢复分升 `numericDisplay`;`SectionHeader.module` 预设收编 6 种 padding 覆盖;`_HealthPanelHeader` 并入 `AppMetricHeader`)· P6' 清理 ✅(零调用 token 删除:`AppRadius.none`、`AppBlur.nav/sticky`、`AppOpacity.focusRing/softScrim`、`AppControlWidths` ×2;裸宽度/断点收编进 `Breakpoints.compactModule/compactContent/formColumn/dialogMax/dialogWide/readingColumn`,settings 760 与 dividend 760 统一至 `contentThreeColumn`;knowledge 状态 wire 展示位全部走 `knowledge_status_labels.dart`,`WIRE_BASELINE` 66→43——余量均为搜索键/负载/文件名等非展示用途)。**范围决定**:单调用 `AppControlWidths` token 保留(与"禁裸数字"护栏冲突,token 即命名);私有度量瓦片余量分属三种形态(cluster 单元/KV 行/图标统计瓦),归并需带 golden 的专项。
