...
void main() {
  ...
  group('ChatRepository — freshness gate', () {
    late ChatHistoryStore store;
    late _FakeApi api;
    late InMemoryAiTraceStore traceStore;
    late ChatRepository repo;

    Future<ChatTracePrepResult> prepWithLocalHlc(String? localHlcText) async {
      const route = RouteContext(path: '/', area: 'home');
      const intent = IntentHint(
        capability: Capability.analyze,
        risk: RiskLevel.info,
      );
    ...
    test('bumps staleReadModels when local HLC is ahead of cloud watermark', () async {
      repo = ChatRepository(
        store: store,
        api: api,
        sessionReader: () => _fakeSession,
        tracePrep: ({required requestId}) =>
            prepWithLocalHlc('00000001700500000000.0001-device'),
        traceStore: traceStore,
      );
      await repo.createSession(ownerUserId: 'user-1');
      final id = (await store.watchSessions('user-1').first).single.id;

      api.script.addAll(const <AiChatEvent>[
        ToolCallEvent(
          id: 't_1',
          name: 'get_monthly_spend_by_category',
          input: <String, Object?>{'year_month': '2026-05'},
        ),
        ToolResultEvent(
          id: 't_1',
          name: 'get_monthly_spend_by_category',
          output: <String, Object?>{
            ...
            'freshness': <String, Object?>{
              ...
              'source_hlc_watermark': '00000001700000000000.0001-server',
              'refreshed_at': '2026-05-12T09:00:00Z',
              'schema_version': 1,
      ...
      final trace = (await traceStore.recent()).single;
      expect(trace.staleReadModels, 1);
    });

    test('does NOT bump when cloud watermark is ahead of local HLC', () async {
      repo = ChatRepository(
        store: store,
        api: api,
        sessionReader: () => _fakeSession,
        tracePrep: ({required requestId}) =>
            prepWithLocalHlc('00000001699000000000.0001-device'),
        traceStore: traceStore,
      );
      await repo.createSession(ownerUserId: 'user-1');
      final id = (await store.watchSessions('user-1').first).single.id;
    ...
    test('no freshness field → counter stays at 0', () async {
      repo = ChatRepository(
        store: store,
        api: api,
        sessionReader: () => _fakeSession,
      ...
      api.script.addAll(const <AiChatEvent>[
        ...
        ToolResultEvent(
          id: 't_1',
          name: 'compute_xirr',
          output: <String, Object?>{'xirr': 0.12},
        ),
        DoneEvent(stopReason: 'end_turn', rounds: 1),
      ]);
      await repo.sendMessage(
