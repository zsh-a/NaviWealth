...
void main() {
  group('decodeSseEvents', () {
    test('decodes a typical text → tool_call → tool_result → done sequence',
        () async {
    ...
    test('handles frames split across chunk boundaries', () async {
      final whole = _frame('text', {'text': '中文测试'}) +
    ...
    test('emits ErrorEvent followed by DoneEvent for failure stream',
        () async {
      final source = _stream([
    ...
    test('flushes a trailing frame missing the blank-line terminator',
        () async {
