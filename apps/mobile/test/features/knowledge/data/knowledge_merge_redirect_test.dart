...
void main() {
  ...
  test('mergeAssumptions re-points Decision.assumptionIds + Experiment.target '
      'and unions evidence; duplicate is tombstoned', () async {
    ...
    final dup = (await repo.findAssumption(ownerUserId: owner, id: 'a-dup'))!;
  ...
  test('mergePrinciples re-points Decision.principleIds', () async {
    ...
    final dup = (await repo.findPrinciple(ownerUserId: owner, id: 'p-dup'))!;
    await repo.mergePrinciples(
      primary: primary,
      duplicates: [dup],
      stamp: stamp,
    );
    ...
    expect(
      (await repo.findPrinciple(ownerUserId: owner, id: 'p-dup'))!.mergedIntoId,
      'p-keep',
    );
    final d = await repo.findDecision(ownerUserId: owner, id: 'd-1');
  ...
  test(
    'mergeDecisions re-points other decisions supersededByDecisionId',
    () async {
      ...
      final dup = (await repo.findDecision(ownerUserId: owner, id: 'd-dup'))!;
      await repo.mergeDecisions(
        primary: primary,
        duplicates: [dup],
        stamp: stamp,
      );
  ...
  test(
    'mergeExperiments unions metrics and tombstones (no inbound refs)',
    () async {
      ...
      final dup = (await repo.findExperiment(ownerUserId: owner, id: 'e-dup'))!;
      ...
      final keep = await repo.findExperiment(ownerUserId: owner, id: 'e-keep');
      expect(keep!.metrics, containsAll(<String>['roi', 'sharpe']));
      expect(
        (await repo.findExperiment(
          ownerUserId: owner,
          id: 'e-dup',
        ))!.mergedIntoId,
        'e-keep',
