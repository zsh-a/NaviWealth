...
void main() {
  group('multi-device sync E2E', () {
    ...
    test('single-device write becomes visible on peer within one cycle', () async {
    ...
    test('offline batch edit replays and reaches peer after reconnect', () async {
      final cluster = SyncCluster();
      final iphone = cluster.addDevice('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa');
      final mac = cluster.addDevice('bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb');
      ...
      expect(await iphone.outbox.depth(), 6,
      ...
      expect(await iphone.outbox.depth(), 0,
      ...
      expect(macA3?.isDeleted, isTrue,
          reason: 'tombstone propagated even when same row was inserted then '
    ...
    test('concurrent same-row writes converge to a single LWW value', () async {
      ...
      final iphoneName = iphone.applier.lookup('accounts', 'A1')!.payload['name'];
      final webName = web.applier.lookup('accounts', 'A1')!.payload['name'];
      expect(iphoneName, equals(webName),
    ...
    test('tombstone hides on peer and does not resurrect on later sync',
        () async {
      final cluster = SyncCluster();
      final iphone = cluster.addDevice('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa');
      final mac = cluster.addDevice('bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb');
      ...
      expect(mac.applier.list('accounts'), isEmpty,
    ...
    test('1000-op backlog drains across paged push/pull without loss',
        () async {
      final cluster = SyncCluster();
      final iphone = cluster.addDevice('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa');
      final mac = cluster.addDevice('bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb');
      ...
      for (var i = 0; i < total; i++) {
        await iphone.writeRow(
          table: 'transactions',
          rowId: 'T${i.toString().padLeft(4, '0')}',
          opType: OpType.insert,
          fieldsDiff: {
            'id': 'T${i.toString().padLeft(4, '0')}',
            'amount': i,
            'note': 'bulk-$i',
          },
        );
      ...
      expect(await iphone.outbox.depth(), 0,
          reason: 'all 1000 ops acked');
      expect(mac.applier.list('transactions').length, total,
      ...
      for (final batch in cluster.api.pushedBatches) {
        expect(batch.length, lessThanOrEqualTo(500),
