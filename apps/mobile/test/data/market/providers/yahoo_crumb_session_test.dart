...
void main() {
  group('YahooCrumbSession.ensureReady', () {
    test('captures Set-Cookie from priming response and fetches crumb', () async {
    ...
    test('falls back to finance.yahoo.com when fc.yahoo.com has no cookies',
        () async {
      final adapter = CannedAdapter()
        ..enqueueRaw(
          'fc.yahoo.com',
          CannedResponse('', status: 404, headers: const {
            Headers.contentTypeHeader: ['text/html'],
          }),
        )
        ..enqueueRaw(
        ...
        ..enqueueRaw(
          'getcrumb',
          CannedResponse('FALLBACK_CRUMB', status: 200, headers: const {
            Headers.contentTypeHeader: ['text/plain'],
          }),
        );
    ...
    test('throws ProviderResponseException when no cookies are obtained',
        () async {
      final adapter = CannedAdapter()
        ..enqueueRaw(
          'fc.yahoo.com',
        ...
        ..enqueueRaw(
          'finance.yahoo.com',
          CannedResponse('', status: 200, headers: const {
            Headers.contentTypeHeader: ['text/html'],
          }),
        );
    ...
    test('coalesces concurrent ensureReady calls', () async {
      final adapter = CannedAdapter()
        ...
        ..enqueueRaw(
          'getcrumb',
          CannedResponse('CONC_CRUMB', status: 200, headers: const {
            Headers.contentTypeHeader: ['text/plain'],
          }),
        );
      ...
      final fcCalls =
          adapter.calls.where((c) => c.uri.host == 'fc.yahoo.com').length;
  ...
  group('YahooCrumbSession outbound headers', () {
    test(
      'sends browser-like UA + Accept headers on getcrumb (Dio BaseOptions '
      'do not merge into hand-built RequestOptions — every request must '
      'carry the headers explicitly)',
      () async {
        final adapter = CannedAdapter()
          ..enqueueRaw(
            'fc.yahoo.com',
          ...
          ..enqueueRaw(
            'getcrumb',
            CannedResponse('HEADERCRUMB', status: 200, headers: const {
              Headers.contentTypeHeader: ['text/plain'],
            }),
          );
        ...
        final crumbCall =
            adapter.calls.firstWhere((c) => c.uri.path.contains('getcrumb'));
  ...
  group('YahooCrumbSession.invalidate', () {
    test('drops state so the next ensureReady reissues the handshake',
        () async {
      final adapter = CannedAdapter()
        ..enqueueRaw(
          'fc.yahoo.com',
          CannedResponse('', status: 404, headers: {
            'set-cookie': ['A1=first'],
            Headers.contentTypeHeader: ['text/html'],
          }),
        )
        ..enqueueRaw(
          'getcrumb',
          CannedResponse('CRUMB_FIRST', status: 200, headers: const {
            Headers.contentTypeHeader: ['text/plain'],
          }),
        )
        ..enqueueRaw(
        ...
        ..enqueueRaw(
          'getcrumb',
          CannedResponse('CRUMB_SECOND', status: 200, headers: const {
            Headers.contentTypeHeader: ['text/plain'],
          }),
        );
