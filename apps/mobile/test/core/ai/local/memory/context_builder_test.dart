...
void main() {
  group('ContextBuilder.build', () {
    ...
    test('classifies records into correct slots', () async {
      ...
      await r.remember(
        _mem(id: 'rule', kind: MemoryKind.procedural),
      );
      await r.remember(
        _mem(
          id: 'decision',
      ...
      await r.recordEvent(EventRecord(
        id: 'event-1',
        type: 'trade_closed',
    ...
    test('related_events is the entity-filtered subset of recent_events',
        () async {
      final b = _builder();
      final r = b.runtime;
      ...
      expect(pack.recentEvents.map((e) => e.id), containsAll(['nvda', 'aapl']));
    ...
    test('ContextPackMemory.toJson exposes all five slots', () async {
      final b = _builder();
      await b.runtime
          .remember(_mem(id: 'pref', kind: MemoryKind.semantic, scope: '*'));
      final pack = await b.build(
        ownerUserId: _kOwner,
        intent: const ContextIntent(),
      );
