...
void main() {
  group('classifyLlmProbeException', () {
    test('401/403 → authFailed', () {
      expect(classifyLlmProbeException(_e(401)).status,
          LlmProbeStatus.authFailed);
      expect(classifyLlmProbeException(_e(403)).status,
          LlmProbeStatus.authFailed);
    });

    test('404 → notFound (bad base url)', () {
      expect(
          classifyLlmProbeException(_e(404)).status, LlmProbeStatus.notFound);
    });
