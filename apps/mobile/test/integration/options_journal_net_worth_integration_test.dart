...
void main() {
  group('Integration: options journal moves net worth (real Drift)', () {
    ...
    test('LEAPS open and close mirror one option lot and cash legs', () async {
      final env = await IntegrationEnv.create();
      final accountRepo = await env.container.read(
        accountRepositoryProvider.future,
      );
      ...
      final openRows = await env.db
          .customSelect(
            ...
            ORDER BY position
            ''',
            variables: [Variable.withString('options:${opened.id}:leapsOpen')],
          )
          .get();
      ...
      final closeRows = await env.db
          .customSelect(
            ...
            ORDER BY position
            ''',
            variables: [Variable.withString('options:${opened.id}:leapsClose')],
          )
          .get();
    ...
    test('LEAPS exercise rolls premium into underlying share basis', () async {
      final env = await IntegrationEnv.create();
      final accountRepo = await env.container.read(
        accountRepositoryProvider.future,
      );
