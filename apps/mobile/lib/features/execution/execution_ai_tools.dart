...
import 'ai_tools/propose_plan_tool.dart';
...
final List<RegisteredDeviceTool> kExecutionToolRegistrations =
    <RegisteredDeviceTool>[
      ...
      _executionTool.propose(const ProposePlanTool()),
...
final List<DeviceTool> kExecutionAssistantDeviceTools =
    registeredAssistantDeviceTools(kExecutionToolRegistrations);
...
const String kExecutionSystemPromptBlock =
    ...
    '- Plan 可通过 horizon 表达短期交付或持续计划；进展记录对用户统一称为 Update。\n'
    '- 查询当前待办用 list_open_actions；定位阻塞用 list_blocked_actions；复盘执行状态与 active Plan 上下文用 summarize_execution_progress。\n'
    '- 当 FinanceOS / HealthOS / KnowledgeOS 的洞察需要落地为执行对象时，优先创建 propose_action；只有明确需要多个步骤或持续跟进时才使用 propose_plan，必要时再使用 propose_progress。'
