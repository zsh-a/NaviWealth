...
class _StudioAssets extends StatelessWidget {
  const _StudioAssets({
    required this.portfolio,
    required this.sleeves,
    required this.tree,
            ...
            child: Column(
              children: [
                for (var index = 0; index < inclusions.length; index++) ...[
                  _CapitalInclusionTile(item: inclusions[index]),
...
class _CapitalInclusionTile extends StatelessWidget {
  const _CapitalInclusionTile({required this.item});
  ...
  Widget build(BuildContext context) {
    ...
    final sourceLabel = assignment.sourceKind == PortfolioCapitalSourceKind.lot
        ? l10n.portfolioStudioIncludedPositionLabel
        : '${assignment.amount ?? Decimal.zero} ${assignment.currency ?? ''}';
    return FTile(
      prefix: Icon(
