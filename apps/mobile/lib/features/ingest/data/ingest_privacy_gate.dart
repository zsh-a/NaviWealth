...
/// device-direct Vision against the user's configured provider (§5.10.10 ②③);
/// whether that is permitted is the user's §5.10.5 privacy posture decision,
/// **not** the pipeline's:
///
///   * `amountsLocal` ("金额完全本地") → provider Vision is off. The most
///     sensitive raw artefact (a receipt image) must never leave the device,
///     so non-device sources are blocked outright.
///   * `amountsAllowed` / `amountsBucketed` → provider Vision is allowed.
///
