...
Future<bool> saveBackupFileImpl(Uint8List bytes, String fileName) async {
  ...
  if (_window.hasProperty('showSaveFilePicker'.toJS).toDart) {
    try {
      final handle = await (_window.callMethodVarArgs(
      ...
      if (handle != null) {
        final writable = await ((handle as JSObject).callMethodVarArgs(
          'createWritable'.toJS,
          <JSAny?>[],
        ) as JSPromise<JSAny?>).toDart;
        if (writable != null) {
          await ((writable as JSObject).callMethodVarArgs('write'.toJS, [
            blob,
          ]) as JSPromise<JSAny?>).toDart;
          await (writable.callMethodVarArgs(
            'close'.toJS,
            <JSAny?>[],
          ) as JSPromise<JSAny?>).toDart;
