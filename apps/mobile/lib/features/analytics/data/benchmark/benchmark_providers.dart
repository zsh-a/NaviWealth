...
final benchmarkComparisonRangeProvider =
...
final benchmarkComparisonSelectionProvider =
    StateProvider<List<BenchmarkIndex>>(
  (ref) => const [BenchmarkIndex.hs300],
);
...
/// curve. Reuses [DashboardTrendBuilder] — once
/// `ReturnsService.portfolioXirr` is wired through Riverpod, the headline
/// annualized return can be overridden via
...
final benchmarkHistorySourceProvider =
    FutureProvider<BenchmarkHistorySource>((ref) async {
...
final benchmarkComparisonResultProvider =
  ...
  final portfolioAsync =
      ref.watch(benchmarkComparisonPortfolioSeriesProvider);
  ...
  for (final index in selection) {
    benchmarks[index] = await ref.watch(benchmarkSeriesProvider(index).future);
  }
