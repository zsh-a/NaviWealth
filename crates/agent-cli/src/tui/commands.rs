...
use agent_core::{AgentRunStore, AgentServices, PROTOCOL_VERSION, RunId, RunRequest};
...
use crate::{
    catalog::load_catalog_registry, config::execution_policy, registry::load_registry,
    tools::CliServices, trace_store::write_store_trace,
};
...
fn show_help(state: &mut TuiState) {
    state.push_log("Commands:");
    state.push_log("  run <agent_id> [json]      run an agent with optional JSON input");
    state.push_log("  tool <name> [json]         call a tool through the active CLI tool services");
    ...
    state.push_log("  clear                      clear the debug output panel");
}

async fn run_agent_command(state: &mut TuiState, rest: &str) -> Result<()> {
    let (agent_id, json_input) = split_name_and_json(rest, "agent id")?;
    let input = parse_json_or_default(json_input, "run input")?;
    state.push_log(format!("$ run {agent_id} {}", compact_json(&input)));

    let registry = match &state.options.catalog_path {
        Some(path) => load_catalog_registry(path.clone()).await?,
    ...
    let outcome = runner
        .run_once(
            ...
            RunRequest {
                ...
                metadata: json!({"source": "agent_tui"}),
            },
        )
        .await
        .into_diagnostic()?;
    ...
    state.push_log(pretty_json(&outcome.result.output));
    Ok(())
}
